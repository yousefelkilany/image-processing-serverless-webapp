...
def handler(event, context):
    ...
    for record in images_records:
        ...
        save_key = upload_key.replace("uploads", f"resized-{width}x{height}", 1)

        logger.info(f"Downloading [[{upload_key}]] from bucket [[{upload_bucket}]].")
        (f"Downloading [[{upload_key}]] from bucket [[{upload_bucket}]].")
        ...
        buffer_size = uploaded_img.getbuffer().nbytes
        uploaded_img.seek(0)

        # logger.debug(f"Downloaded {buffer_size} bytes into memory buffer.")
        # first_16_bytes = uploaded_img.read(16)
        # uploaded_img.seek(0)
        # logger.debug(f"First 16 bytes of the file data: {first_16_bytes}")
